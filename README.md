...
I also run [OVN](https://www.ovn.org/en/) to play with networking on some VMs.
...
- `nginx/default.nix` is a derivation that contains a container image of nginx
  ...
  images. This has to be cross-compiled for ARM64.
...
deploy secrets. Disk reformatting is triggered by passing
`homelab.provision_disks=true` on the kernel cmdline.
...
Certificates are automatically generated/renewed with ACME.sh for web services.
DNS provider credentials are provided as secrets.
